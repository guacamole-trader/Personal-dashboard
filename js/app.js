...
async function loadDailyMsg() {
  try {
    ...
    if (data && v('daily-msg')) v('daily-msg').value = data.message || '';
  } catch(e) {}
}
...
function setProgress(sec, done, total) {
  const statEl = v(sec+'-stat'), barEl = v(sec+'-bar');
...
// ── DRAG TO REORDER ───────────────────────────────────────────────────────
const dragInstances = {};
const sectionTables = { todo:'todos', books:'books', videos:'videos', restaurants:'restaurants', travel:'travel', groceries:'groceries', subs:'subscriptions', budget:'budget', car:'car_maintenance' };
...
function initDrag(sec) {
  ...
  dragInstances[sec] = Sortable.create(tbody, {
    animation: 150, handle: '.drag-handle', ghostClass: 'drag-ghost', chosenClass: 'drag-chosen',
    onEnd: async () => {
      const rows = Array.from(tbody.querySelectorAll('tr[data-id]'));
      await Promise.all(rows.map((row,idx) => dbUpdate(table, row.dataset.id, { sort_order: idx })));
    }
  });
}
...
async function renderTodo() {
  ...
  updateBadge('todo',rows); setProgress('todo',rows.filter(r=>r.done).length,rows.length);
...
async function renderBooks() {
  ...
  updateBadge('books',rows); setProgress('books',rows.filter(r=>r.status==='Read').length,rows.length);
...
async function renderVideos() {
  ...
  updateBadge('videos',rows); setProgress('videos',rows.filter(r=>r.status==='Done').length,rows.length);
...
async function renderRestaurants() {
  ...
  updateBadge('restaurants',rows); setProgress('restaurants',rows.filter(r=>r.status==='Tried'||r.status==='Favorite').length,rows.length);
...
async function renderTravel() {
  ...
  updateBadge('travel',rows); setProgress('travel',rows.filter(r=>r.status==='Visited').length,rows.length);
...
async function renderGroceries() {
  ...
  if(statEl) statEl.textContent = `${rows.filter(r=>r.status==='bought').length} of ${rows.length} bought`;
...
async function renderCar() {
  ...
  updateBadge('car',rows); setProgress('car',rows.filter(r=>r.status==='Done').length,rows.length);
